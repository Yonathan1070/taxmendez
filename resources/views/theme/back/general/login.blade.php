...
<head>
    ...
    <title>{{Lang::get('messages.Login')}} - {{Lang::get('messages.TaxMendez')}}</title>
    <!-- Bootstrap Core CSS -->
...
<body>
    ...
    <!-- ============================================================== -->
    <section id="wrapper">
        <div class="login-register" style="background-image:url({{asset("assets/back/images/background/login-register.jpg")}});">
            <div class="login-box card">
                <div class="card-body">
                    @if ($errors->any())
                        <x-alert tipo="danger" :mensaje="$errors" />
                    @endif
                    <form class="form-horizontal form-material" id="loginform" action="{{route('login')}}" method="POST" novalidate>
                        @csrf
                        <h3 class="box-title m-b-20">{{Lang::get('messages.Login')}}</h3>
                        <div class="form-group">
                            <div class="col-xs-12 controls">
                                <input class="form-control" type="text" required placeholder="{{Lang::get('messages.UserName')}}" id="USR_Nombre_Usuario" name="USR_Nombre_Usuario" data-validation-required-message="{{Lang::get('messages.Required')}}" pattern="(?=.*?[A-Z0-9])([a-zA-Z0-9]{7,})" data-validation-pattern-message="{{Lang::get('messages.RegexUserName')}}">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-xs-12 controls">
                                <input class="form-control" type="password" required placeholder="{{Lang::get('messages.Password')}}" id="password" name="password" data-validation-required-message="{{Lang::get('messages.Required')}}" pattern="(?=.*?[A-Z0-9!#$%&/()=?¡\\¿¨´*+~{}^`,;.:_-])([a-zA-Z0-9!#$%&/()=?¡\\¿¨´*+~{}^`,;.:_-]{7,})" data-validation-pattern-message="{{Lang::get('messages.RegexPassword')}}">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="d-flex no-block align-items-center">
                                <div class="checkbox checkbox-primary p-t-0">
                                    ...
                                    <label for="checkbox-signup"> {{Lang::get('messages.RememberMe')}} </label>
                                </div> 
                                <div class="ml-auto">
                                    <a href="javascript:void(0)" id="to-recover" class="text-muted">
                                        <i class="fa fa-lock m-r-5"></i> {{Lang::get('messages.ForgotPassword')}}
                                    </a> 
                                </div>
                            </div>
                        </div>
                        <div class="form-group text-center m-t-20">
                            <div class="col-xs-12">
                                <button class="btn btn-info btn-lg btn-block text-uppercase waves-effect waves-light" type="submit">{{Lang::get('messages.Login')}}</button>
                            </div>
                        </div>
                        {{--<div class="row">
